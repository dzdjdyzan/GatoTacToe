...
public class GatoTacToeGameUIManager : MonoBehaviour
{
    ...
    void RefreshDisplay()
    {
        ...
        player1TimerText.text = $"Player X\n{FormatTime(p1Elapsed)}";
        player2TimerText.text = $"Player O\n{FormatTime(p2Elapsed)}";
        totalTimerText.text = $"{FormatTotalTime(totalElapsed)}";

        // --- Turn counts ---
        //player1TurnsText.text = $"X Turns\n{game.GetPlayer1MoveCount()}";
        //player2TurnsText.text = $"Y Turns\n{game.GetPlayer2MoveCount()}";
    }
